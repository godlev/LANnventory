...
func buildApplyUpdateScript(params applyScriptParams) string {
	...
	builder.WriteString(`  now="$(date -u +%Y-%m-%dT%H:%M:%SZ)"` + "\n")
	...
	builder.WriteString(`  printf '{"attemptId":%s,"status":"%s","stage":"%s","previousVersion":%s,"targetVersion":%s,"startedAt":%s,"updatedAt":"%s","completedAt":"%s","backupPath":%s,"backupCreated":%s,"failedStage":"%s","error":"%s","serviceRestored":%s,"systemdUnit":%s}\n' "$attempt_json" "$status" "$stage" "$previous_json" "$target_json" "$started_json" "$now" "$completed_at" "$backup_json" "$backup_created" "$failed_stage" "$error_message" "$service_restored" "$unit_json" > "$tmp"` + "\n")
