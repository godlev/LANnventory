...
func TestPhase35FinalBetaReleaseSelection(t *testing.T) {
	releases := []release{
		{
			...
			HTMLURL:     "https://github.com/godlev/LANnventory/releases/tag/v0.1.0-beta.7",
		},
		{TagName: "v0.1.0-beta.6.uat.9", Prerelease: true, Draft: false},
		{TagName: "v0.1.0-beta.6.uat.4", Prerelease: true, Draft: false},
		{TagName: "v0.1.0-beta.6", Prerelease: true, Draft: false},
	}
	...
	for _, installed := range []string{
		...
		status, selected, ok := buildStatus(installed, BetaChannel, releases, time.Time{}, false)
		if !ok {
			t.Fatalf("Beta channel did not select beta.7 for installed %s", installed)
		}
		if selected.TagName != "v0.1.0-beta.7" {
			t.Fatalf("installed %s selected %q, want v0.1.0-beta.7", installed, selected.TagName)
		}
		if !status.Available || status.LatestVersion != "0.1.0-beta.7" {
			t.Fatalf("installed %s status = %+v, want available beta.7", installed, status)
		}
	}
	...
	if ok || selected.TagName != "" || stableStatus.Available {
		t.Fatalf("Stable channel exposed beta.7 prerelease: status=%+v selected=%+v ok=%v", stableStatus, selected, ok)
	}
}

func TestPhase358FinalBetaReleaseSupersedesUAT2(t *testing.T) {
	releases := []release{
		{TagName: "v0.1.0-beta.9", Prerelease: true, Draft: false},
		{TagName: "v0.1.0-beta.8.uat.2", Prerelease: true, Draft: false},
		{TagName: "v0.1.0-beta.8", Prerelease: true, Draft: false},
		{TagName: "v0.1.0-beta.7", Prerelease: true, Draft: false},
	}
	...
	for _, installed := range []string{
		...
		if selected.TagName != "v0.1.0-beta.9" {
			t.Fatalf("installed %s selected %q, want v0.1.0-beta.9", installed, selected.TagName)
		}
		if !status.Available || status.LatestVersion != "0.1.0-beta.9" {
			t.Fatalf("installed %s status = %+v, want available beta.9", installed, status)
		}
	}
}
...
func TestPhase359UAT1SelectionFromBeta9(t *testing.T) {
	releases := []release{
	...
	if !ok || selected.TagName != "v0.1.0-beta.9.uat.1" {
		t.Fatalf("Beta selected %+v ok=%v, want Phase 35.9 UAT1", selected, ok)
	}
	if !status.Available || status.LatestVersion != "0.1.0-beta.9.uat.1" {
		t.Fatalf("Beta status = %+v, want Phase 35.9 UAT1 available", status)
	}
...
func TestPhase359UAT3SelectionFromUAT2(t *testing.T) {
	releases := []release{
	...
	if !ok || selected.TagName != "v0.1.0-beta.9.uat.3" {
		t.Fatalf("Beta selected %+v ok=%v, want Phase 35.9 UAT3", selected, ok)
	}
	if !status.Available || status.LatestVersion != "0.1.0-beta.9.uat.3" {
		t.Fatalf("Beta status = %+v, want Phase 35.9 UAT3 available", status)
	}
	...
	next, ok := selectLatestRelease(future, BetaChannel)
	if !ok || next.TagName != "v0.1.0-beta.10" {
		t.Fatalf("future beta.10 should supersede Phase 35.9 UAT3, got %+v ok=%v", next, ok)
	}
}


func TestPhase359FinalBeta10SupersedesUAT3(t *testing.T) {
	releases := []release{
		{TagName: "v0.1.0-beta.10", Prerelease: true, Draft: false},
		{TagName: "v0.1.0-beta.9.uat.3", Prerelease: true, Draft: false},
		{TagName: "v0.1.0-beta.9.uat.2", Prerelease: true, Draft: false},
	...
	for _, installed := range []string{
		...
		if selected.TagName != "v0.1.0-beta.10" {
			t.Fatalf("installed %s selected %q, want v0.1.0-beta.10", installed, selected.TagName)
		}
		if !status.Available || status.LatestVersion != "0.1.0-beta.10" {
			t.Fatalf("installed %s status = %+v, want available beta.10", installed, status)
		}
	}

	stableStatus, selected, ok := buildStatus("v0.1.0-beta.9.uat.3", StableChannel, releases, time.Time{}, false)
